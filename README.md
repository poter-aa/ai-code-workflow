...
> 一个基于 AI 的代码工作流自动化系统，通过命令驱动的方式管理从需求创建到代码交付的完整生命周期。

## 📖 项目简介

AI Code Workflow 是一个智能化的代码工作流管理系统，旨在通过标准化的命令和自动化脚本，将需求管理、任务规划、代码开发、进度跟踪和代码交付等环节串联起来，形成完整的自动化工作流。

### 核心特性

- 🎯 **命令驱动**：通过 `/issue-create`、`/issue-plan` 等命令管理整个开发流程
- 🤖 **AI 自动化**：集成 Claude Code 和 Cursor，支持 24 小时无人值守执行
- 📊 **进度可视**：自动生成和更新进度文档，实时跟踪任务状态
- 🔄 **完整闭环**：从需求创建到 MR 提交，全流程自动化
